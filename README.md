...
- **区域修补**：用周围像素盖掉指定区域，可去污点、日期戳或他人的水印（近似修补，非还原）
...
> 需要说明：这个功能针对的是「自己加错了要重来」这类场景。要处理别人加的水印，
> 用下面的 [`imprint erase`](#imprint-erase)——那是猜测性修补，不是还原。

### `imprint erase`

修补图片的指定区域——用周围的像素把里面的内容盖掉。这是通用的修图能力，去污点、
日期戳、电线、路人都能用，去水印只是其中一种用法。

**和 `remove` 是两件事**：`remove` 知道当初的水印参数，做的是精确逆运算，结果是
*还原*；`erase` 对水印一无所知，只能拿周围的像素猜中间该是什么，结果是*近似*。

```bash
# 圈出水印所在的矩形，再把框里真正属于水印的笔画挑出来
imprint erase -i photo.jpg -o clean.png --region 960,655,305,55 --color "#FFFFFF"

# 平铺满屏的淡水印：不给 region，全图按颜色选，容差调大
imprint erase -i photo.jpg -o clean.png --color "#FFFFFF" --tolerance 250

# 用蒙版图精确指定：蒙版上白色的地方会被修补
imprint erase -i photo.jpg -o clean.png --mask mask.png
```
...
| `-i, --input <PATH>...` | 输入文件或目录 |
| `-o, --output <PATH>` | 输出文件或目录 |
| `--region <X,Y,W,H>` | 要修补的矩形，像素坐标，可重复给多个 |
| `--mask <FILE>` | 蒙版图，白色处需修补，尺寸须与原图一致 |
| `--mask-threshold <N>` | 蒙版的亮度阈值，默认 `128` |
| `--color <HEX>` | 只修补接近该颜色的像素 |
| `--tolerance <N>` | `--color` 的容差，三通道差值之和，`0~765`，默认 `200` |
| `--grow <N>` | 选区外扩的像素数，默认 `2`，用来盖住抗锯齿的边缘 |
| `--radius <N>` | 采样半径，默认 `4`；越大越平滑也越糊 |
| `--format <FORMAT>` | 默认 `png` |

三种选区来源可以叠加：`--region` 与 `--mask` 取并集构成初始范围，`--color` 在该范围内
*收窄*（范围为空时退化为全图按颜色选取），最后 `--grow` 向外扩张。

#### 实测效果

样张是 1280×720 的合成图，但相邻像素平均差做到 7.85，与真实照片同量级——
平滑的渐变图会测出高得多也没有参考价值的数字。下表是水印区域相对原图的 PSNR：

| 水印形态 | 处理前 | 只给 `--region` | `--region` + `--color` |
| --- | --- | --- | --- |
| 角落文字，α=0.55 | 21.7 dB | 27.7 dB | **37.3 dB** |
| 角落文字，α=1.0（不透明） | 16.5 dB | 26.5 dB | **36.4 dB** |
| 画面中央大字，α=0.5 | 24.2 dB | 23.7 dB（更差） | **40.1 dB** |
| 平铺满屏，α=0.45 | 25.4 dB | 不适用 | **35.1 dB**（`--tolerance 250`） |

两个要点：

- **`--color` 是效果的分水岭**，差出 9~16 dB。只给 `--region` 会把框里的背景一并糊掉；
  框一旦大到几百像素见方，糊出来的结果比留着水印还难看——表中第三行就是这个情况。
- **`--tolerance` 要按水印浓淡调**。水印越淡，它与背景混合后离自身颜色越远，需要的
  容差越大：实测 α=1.0 时 60 就够，α=0.55 时 200 最优，α=0.45 时要 250。默认值 200
  就是这么定的。调到选中背景就会反向变差（α=0.45 的样张在 350 时掉回 26 dB）。

**限制：**

- **修补是猜测，不是还原。** 背景越平滑猜得越准；水印压在复杂纹理、文字或人脸上时，
  补出来的一定和原样不同。
- **大面积覆盖只能淡化。** 周围的已知像素本身也被水印盖住时，没有可参考的信息。
- **坐标要自己量。** 用任意看图软件读出水印的像素坐标填进 `--region`。
- 选区一个像素都没命中时会提示「输出与原图一致」，不会让你以为处理过了。
